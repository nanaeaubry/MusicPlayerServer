...
public class Chord extends java.rmi.server.UnicastRemoteObject implements ChordMessageInterface {
	...
	private void increasePtPCount(String fileName) {
		...
		if (pagesToProcess.containsKey(fileName)) {
			...
			// System.out.println(TAG + "(): count = " + pagesToProcess.get(fileName) ); //
			// DEBUG
		} else {
	...
	// if tree map already contains the key then combine received page with current
	// page
	...
	public void arePagesSent(long source, String fileName, Boolean state, int n) throws RemoteException {
		...
		if (n == 0) {
			...
			if (result == 0)// if result == 0 they are equal // this is the start of the chord
			{
				...
				// else the previous state was positive , check the local state and pass to the
				// successor.
				else// state == true //
				{
					System.out.println(TAG + "(): id: " + this.guid + ": state: " + this.sentState);
	...
	private Boolean isPagesToProcessZero(String fileName) {
		...
		// System.out.println(TAG + "("+ fileName +"): result = " +
		// pagesToProcess.get(fileName)); // DEBUG
	...
	// public void mapContext(int page, Mapper mapper, ChordMessageInterface
	// coordinator, String file) throws RemoteException
	// {

	// }

	// public void reduceContext(int page, Mapper reducer, ChordMessageInterface
	// coordinator, String file) throws RemoteException
	// {
